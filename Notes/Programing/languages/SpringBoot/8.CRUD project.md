...
## Soft delete
till now, have did hard delete -->remove directly from DB.
will mark true to column of `isDeleted` based on it will do other crud operations.
in entity class add new property `isDelete`.`private boolean isDeleted;`
while making new entry
```java
    public Student getStudent(Long id){
        Optional<Student> studentRes=studentRepository.findById(id);
        if(studentRes.isEmpty()) return null;
        if(studentRes.get().getIsDeleted()) return null;
        if(studentRes.isPresent()) return studentRes.get();
        return null;
    }
```
use patch mapping for soft delete(patch used when changing/patching one entry from a table)
```java
    @PatchMapping("/{id}")
...
```java
    public boolean softDelete(Long id){
        ...
        if(studentExisting.isEmpty()) return false;
        Student existing=studentExisting.get();
        if(existing.getIsDeleted()) return false; // already soft deleted
        existing.setDeleted(true);
        studentRepository.save(existing);
        return true;
    }
```
changes in other service methods
```java
    public Student createStudent(Student studentReq){
        studentReq.setDeleted(false);
        studentReq.setId(ctn);
        ctn++;
        Student studentRes = studentRepository.save(studentReq);
        return studentRes;
    }
```
is deleted is internal key thus, even if send create with is deleted true make it false.
![[Pasted image 20260719125804.png]]
for getting student need to consider delete key.but there is no such in-built method for it.
JPA can make a method if we name it in a way using standard naming then, JPA will make it at run time.
```java
    public Student getStudent(Long id){
        Optional<Student> studentRes=studentRepository.findByIdAndIsDeletedIsFalse(id);
        if(studentRes.isEmpty()) return null;
        return studentRes.get();
    }
```
can key add more ![[Pasted image 20260719133330.png]].
need to declare it in interface
```java
@Repository
public interface StudentRepository extends JpaRepository<Student,Long> {
    public Optional<Student> findByIdAndIsDeletedIsFalse(Long id);
}
```
works![[Pasted image 20260719133709.png]]
similarly for all
```java
    public Student getStudent(Long id){
        Optional<Student> studentRes=studentRepository.findByIdAndIsDeletedIsFalse(id);
    ...
    public Student updateStudent(Long id,Student studentReq){
        Student studentExisting = studentRepository.findByIdAndIsDeletedIsFalse(id).get();
        if(studentExisting==null) return null;
        Student studentStore=studentExisting;
        studentStore.setName(studentReq.getName());
        studentStore.setRollNo(studentReq.getRollNo());
        ...
        studentStore.setEmail(studentReq.getEmail());
        studentStore.setSubject(studentReq.getSubject());
        studentStore.setDeleted(false);
        studentRepository.save(studentStore);
        return studentStore;
    }
```
for hard delete let can hard delete any row => thus current implementation is correct.
and soft delete can be eased by to
```java
    public boolean softDelete(Long id){
        Optional<Student> studentExisting = studentRepository.findByIdAndIsDeletedIsFalse(id);
        if(studentExisting.isEmpty()) return false;
        Student existing = studentExisting.get();
        existing.setDeleted(true);
        studentRepository.save(existing);
...
```
JPA => can make query based on name of the function and declaration.
General syntax
```java
...
```java
    public List<Student> getAllStudent(){
        List<Student> studentRes=studentRepository.findByIsDeletedIsFalse();
        return studentRes;
    }
```
id should be handle by Service not send by user.
can do 
```java
@Service
public class StudentService {
...
public class Student {
    ...
    private Long id;
```
problem with both approach is it starts as new table count from 1 to n.
![[Pasted image 20260719141603.png]]
`DDL`(data definition language) command of auto increment.
if make it get id via request params `localhost:8080/api/students?id=1`.
this is used when more than 1 parameter.
```java
    @GetMapping("/get")
...
```
![[Pasted image 20260719143839.png]]
